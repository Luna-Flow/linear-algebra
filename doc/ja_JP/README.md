...
## v0.4.1 - ベンチマークとカーネル保守

このドキュメントは現在の **v0.4.1** リポジトリ状態を説明します。この保守リリースでは
`0.4.0` の検査付き行列 API を維持しつつ、ベンチマーク精度、公開自動化、可変行列カーネル性能を改善しました。

### 保守変更

- `mutable.Matrix::unchecked_matmul` は、形状互換性をすでに保証した呼び出し側とベンチマークのホットパス向けに公開 API になりました。
- 検査付きの `mutable.Matrix` 乗算は引き続き次元を検証し、その後 unchecked カーネルへ委譲します。
- 可変行列乗算、LU の後続更新、Cholesky の内積ループは Native、JS、Wasm、Wasm GC の各バックエンドで重点的に展開されています。
- ベンチマークの乗算ケースは unchecked ホットパスを直接測定し、反復される形状検査が結果を支配しないようにしました。
- `bench/datasets/cases/*.json` は追跡対象ではなく、必要時に生成されるローカル成果物として文書化されています。
- 公開 workflow は `moon.mod` のパッケージバージョンを直接読み取り、手入力のリリースバージョンを不要にしました。
...
| バージョン | 日付 | 状態 | 説明 |
| --- | --- | --- | --- |
| `0.4.1` | 2026-07-07 | 現在のリポジトリ版 | ベンチマーク測定を調整し、公開 unchecked 可変行列乗算を追加し、可変行列ホットループと公開手順を改善 |
...
- **以前のリリース叙述（0.2.11）**:
  ...
  - リリースチェックリスト、ベンチマーク文書、パッケージ概要、多言語 README は `0.2.11` のリリースストーリーに整合しています。
