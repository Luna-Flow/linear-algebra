...
- **高级操作**：包含 行列式、inverse、rank、Cholesky decomposition、eigen 相关例程、row elimination、transpose view，以及矩阵/向量转换。
...
- **正确性优先**：当前覆盖范围包括不可变语义检查、跨包一致性检查、行列式/rank/inverse 对齐，以及数值行为回归测试。
...
- **更早版本主叙事（0.2.11）**：
  ...
  - 公共数值 API 已围绕 `Field` / `Num` / `Tolerance` 对齐，不可变 行列式 的文档也已同步到简化后的约束集合。
...
- **算法与稳定性（0.2.8）**：
  - 引入了 行列式、inverse、rank、eigen 相关能力所依赖的 LU / QR 分解支持。
  - 将 行列式 与 rank 的行为推进到更稳定的消元实现方向。
